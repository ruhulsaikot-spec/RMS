...
export default function NewApprovalGroupPage() {
  ...
  const [members, setMembers] = useState<{ id: string; employee: string; designation: string; department: string }[]>([]);
  const [selectedEmployees, setSelectedEmployees] = useState<string[]>([]);
  const [employeeSearch, setEmployeeSearch] = useState("");
  const [groupName, setGroupName] = useState("");
  const [approvalMethod, setApprovalMethod] = useState("Any One Approves");
  ...
  const handleAddMember = () => {
    ...
    const newMembers = selectedEmployees
      .map((id) => employeeOptions.find((x) => x.id === id))
      .filter(Boolean)
      .filter((e: any) => !members.some((m) => m.id === e.id))
      .map((e: any) => ({
        id: e.id, employee: e.name,
  ...
  const handleCreate = async () => {
    if (!groupName.trim()) { toast.error("Approval group name is required."); return; }
    try {
      setSaving(true);
      const payload = {
        group_code: groupName.trim().toUpperCase().replace(/\s+/g, "_"),
        group_name: groupName.trim(),
        approval_method: approvalMethod === "Any One Approves" ? "ANY_ONE" : "ALL",
        description: description.trim(),
        is_active: groupStatus === "Active",
      };
      const response = await approvalGroupService.createApprovalGroup(payload);
      for (const member of members) {
        await approvalGroupService.addMember({ approval_group_id: response.id, employee_id: member.id });
      }
  ...
  return (
    <PermissionGuard permission="approval_group:create">
      <main className="relative flex min-h-screen overflow-hidden bg-gradient-to-b from-[#030B1F] to-[#06153C] text-white">
        ...
        <div className="relative z-10 flex min-h-screen w-full">
          ...
          <section className="flex-1 flex flex-col">
            <Topbar title="Create Approval Group" subtitle="Set up workflow approval teams" />
                  ...
                  <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-6 backdrop-blur-xl">
                    <div className="mb-5 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-white/40">Configure approval group details</p>
                      </div>
                    </div>
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      <div>
                        <label className="mb-1.5 block text-xs font-medium text-white/60">Group Code</label>
                        <input type="text" value={groupName ? groupName.trim().toUpperCase().replace(/\s+/g, "_") : "Auto Generated"}
                          disabled className="w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-xs text-white/40 cursor-not-allowed" />
                      </div>
                      <div>
                        ...
                        <select value={approvalMethod} onChange={(e) => setApprovalMethod(e.target.value)} className={selectClass}>
                          <option value="Any One Approves" className="bg-[#0d1f40]">Any One Approves</option>
                          <option value="All Must Approve" className="bg-[#0d1f40]">All Must Approve</option>
                        </select>
                      </div>
                      <div>
                      ...
                      <div className="md:col-span-2">
                        ...
                        <textarea rows={3} value={description} onChange={(e) => setDescription(e.target.value)}
                          placeholder="Brief description of this approval group..."
                    ...
                    <div className="mb-5 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-white/40">{members.length} member(s) added</p>
                        </div>
                      </div>
                          ...
                          {members.length === 0 ? (
                            <tr>
                              <td colSpan={4} className="py-10 text-center">
                                ...
                                <button onClick={() => setShowMemberModal(true)}
                                  className="mt-2 text-xs text-cyan-400 hover:text-cyan-300">+ Add your first member</button>
                              </td>
                            </tr>
                          ) : members.map((member) => (
                ...
                {currentStep === 3 && (
                  <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-6 backdrop-blur-xl">
                    <div className="mb-5">
                      ...
                      <p className="text-xs text-white/40">Review information before creating</p>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-5">
                      ...
                      <div className="rounded-2xl border border-blue-500/20 bg-blue-500/10 p-3">
                        ...
                        <p className="mt-1 text-xs font-semibold text-blue-300">{approvalMethod}</p>
                      </div>
                    ...
                    <div className="mt-5 flex justify-end gap-2">
                      ...
                      <button onClick={handleCreate} disabled={saving}
                        ...
                        {saving ? "Creating..." : "Create Approval Group"}
                      </button>
                    </div>
                  </div>
                )}
                  ...
                  <label key={employee.id} className="flex cursor-pointer items-center gap-3 border-b border-white/5 p-3 hover:bg-white/5 transition-colors">
                    ...
                    <input type="checkbox" checked={selectedEmployees.includes(employee.id)}
                      onChange={(e) => {
                        ...
                        else setSelectedEmployees(selectedEmployees.filter((id) => id !== employee.id));
                      }}
                      className="hidden" />
