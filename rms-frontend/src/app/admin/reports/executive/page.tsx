...
export default function ExecutiveReportPage() {
  ...
  const fetchReport = async (f = filters) => {
    try {
      ...
      const res = await apiClient.get(`/reports/executive?${params.toString()}`);
      setRows(res.data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleReset = () => {
    const reset = { department_id: "", date_from: "", date_to: "" };
    setFilters(reset);
    fetchReport(reset);
  };

  const exportCSV = () => {
    const headers = ["Department", "Total Claims", "Total Requested", "Total Verified", "Total Paid", "Total Rejected", "Pending Amount"];
    const csvRows = [
      headers.join(","),
      ...rows.map(r => [
        `"${r.department}"`,
  ...
  const totals = rows.reduce((acc, r) => ({
    ...
    total_requested: acc.total_requested + r.total_requested,
    total_verified: acc.total_verified + r.total_verified,
  ...
  }), { total_claims: 0, total_requested: 0, total_verified: 0, total_paid: 0, total_rejected: 0, pending_amount: 0 });

  return (
    <PermissionGuard permission="report:executive">
    <main className="relative flex min-h-screen overflow-hidden bg-gradient-to-b from-[#030B1F] to-[#06153C] text-white">
      ...
      <div className="relative z-10 flex min-h-screen w-full">
        ...
        <section className="flex-1 flex flex-col">
          <Topbar title="Reports" subtitle="Executive Report" />
          <div className="p-6">
            <div className="mx-auto max-w-7xl space-y-5">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-xl font-semibold text-white">Executive Report</h1>
                  <p className="mt-1 text-xs text-white/60">Department-wise claim summary</p>
                </div>
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-white/10 bg-white/[0.04]">
                        ...
                        <th className="px-4 py-3 text-right font-semibold text-white/60">Verified (৳)</th>
                        <th className="px-4 py-3 text-right font-semibold text-white/60">Paid (৳)</th>
                        <th className="px-4 py-3 text-right font-semibold text-white/60">Rejected (৳)</th>
                        <th className="px-4 py-3 text-right font-semibold text-white/60">Pending (৳)</th>
                      </tr>
                    </thead>
                    <tbody>
                      {loading ? (
                        ...
                        <tr key={i} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                          ...
                          <td className="px-4 py-3 text-right text-cyan-300 font-bold">{r.total_claims}</td>
                          <td className="px-4 py-3 text-right text-white">৳ {Number(r.total_requested).toLocaleString()}</td>
                          ...
                          <td className="px-4 py-3 text-right text-red-300">৳ {Number(r.total_rejected).toLocaleString()}</td>
                          <td className="px-4 py-3 text-right text-yellow-300">৳ {Number(r.pending_amount).toLocaleString()}</td>
                        </tr>
                      ))}
                    </tbody>
                    {rows.length > 0 && (
                      <tfoot>
                        <tr className="border-t border-white/10 bg-white/[0.04]">
                          ...
                          <td className="px-4 py-3 text-right text-xs font-bold text-cyan-300">{totals.total_claims}</td>
                          <td className="px-4 py-3 text-right text-xs font-bold text-white">৳ {Number(totals.total_requested).toLocaleString()}</td>
                          <td className="px-4 py-3 text-right text-xs font-bold text-blue-300">৳ {Number(totals.total_verified).toLocaleString()}</td>
                          <td className="px-4 py-3 text-right text-xs font-bold text-green-300">৳ {Number(totals.total_paid).toLocaleString()}</td>
                          <td className="px-4 py-3 text-right text-xs font-bold text-red-300">৳ {Number(totals.total_rejected).toLocaleString()}</td>
                          <td className="px-4 py-3 text-right text-xs font-bold text-yellow-300">৳ {Number(totals.pending_amount).toLocaleString()}</td>
                        </tr>
                      </tfoot>
                    )}
                  </table>
