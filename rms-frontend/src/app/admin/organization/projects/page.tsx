...
export default function ProjectsPage() {
  ...
  const openEdit = (p: any) => {
    setEditingId(p.id);
    setProjectForm({ code: p.code, name: p.name, status: p.status });
    setShowModal(true);
  };

  const openNew = () => { setEditingId(null); setProjectForm(emptyForm); setShowModal(true); };
  const closeModal = () => { setShowModal(false); setEditingId(null); setProjectForm(emptyForm); };
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      if (editingId) {
        await apiClient.put(`/projects/${editingId}`, {
          code: projectForm.code, name: projectForm.name,
          is_active: projectForm.status === "Active",
        });
  ...
  const handleDelete = async () => {
    try {
      await apiClient.delete(`/projects/${deleteId}`);
      await loadProjects();
      ...
      <main className="relative flex min-h-screen overflow-hidden bg-gradient-to-b from-[#030B1F] to-[#06153C] text-white">
        ...
        <div className="relative z-10 flex min-h-screen w-full">
          ...
          <section className="flex-1 flex flex-col">
            <Topbar title="Projects" subtitle="Manage organizational projects" />
            ...
            <div className="flex-1 p-5 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/10 p-4">
                  ...
                  <p className="mt-2 text-2xl font-bold text-cyan-300">{projects.length}</p>
                </div>
                <div className="rounded-2xl border border-green-500/20 bg-green-500/10 p-4">
                  <p className="text-xs text-white/60">Active</p>
                  <p className="mt-2 text-2xl font-bold text-green-300">{projects.filter((p) => p.status === "Active").length}</p>
                </div>
                <div className="rounded-2xl border border-red-500/20 bg-red-500/10 p-4">
                  <p className="text-xs text-white/60">Inactive</p>
                  <p className="mt-2 text-2xl font-bold text-red-300">{projects.filter((p) => p.status === "Inactive").length}</p>
                </div>
              </div>
              ...
              <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] backdrop-blur-xl">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/10">
                      {["Code", "Project Name", "Status", "Action"].map((h, i) => (
                  ...
                  <tbody>
                    {paginatedProjects.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="py-12 text-center text-xs text-white/40">No projects found</td>
                      </tr>
                    ) : paginatedProjects.map((p) => (
                      <tr key={p.id} className="border-b border-white/5 hover:bg-white/[0.03] transition-colors">
                        <td className="px-4 py-3 text-xs font-semibold text-cyan-300">{p.code}</td>
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="flex justify-end gap-1.5">
                            <ActionGuard permission="project:update">
                            ...
                            <ActionGuard permission="project:delete">
                ...
                <div className="flex items-center justify-between border-t border-white/5 px-4 py-3">
                  <p className="text-xs text-white/40">
                    Showing {Math.min(startIndex + 1, filteredProjects.length)}–{Math.min(startIndex + recordsPerPage, filteredProjects.length)} of {filteredProjects.length}
                  </p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="w-full max-w-md rounded-3xl border border-white/10 bg-[#0d1f40] p-6 shadow-[0_25px_60px_rgba(0,0,0,0.5)]">
              <div className="mb-5 flex items-center justify-between">
                <div>
                  <h2 className="text-base font-bold text-white">{editingId ? "Update Project" : "New Project"}</h2>
                  <p className="text-xs text-white/40">{editingId ? "Update project information" : "Fill in project details"}</p>
                </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-white/60">Project Code <span className="text-red-400">*</span></label>
                  <input value={projectForm.code}
                    onChange={(e) => setProjectForm({ ...projectForm, code: e.target.value })}
                    placeholder="e.g. PRJ-001" className={inputClass} />
                </div>
                <div>
                  <label className="mb-1.5 block text-xs font-medium text-white/60">Project Name <span className="text-red-400">*</span></label>
                  <input value={projectForm.name}
                    onChange={(e) => setProjectForm({ ...projectForm, name: e.target.value })}
                    placeholder="e.g. Digital Transformation" className={inputClass} />
                </div>
                {editingId && (
                  <div>
        ...
        {deleteId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="w-full max-w-sm rounded-3xl border border-red-500/20 bg-[#0d1f40] p-6 shadow-[0_25px_60px_rgba(0,0,0,0.5)]">
              ...
              <p className="mt-2 text-sm text-white/60">Are you sure you want to delete this project?</p>
